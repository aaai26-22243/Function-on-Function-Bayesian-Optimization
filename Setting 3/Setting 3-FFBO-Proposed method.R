...
for(j.ffgp in 1:2){
  ...
  for(i.ffgp in 1:m){
    ...
    ucb.ffgp <- function(dis.pre){
      ...
      cov.g = abs((sigma2*c)*(1-t(R.pre)%*%sol.Sigma.g%*%R.pre)+1e-6)
    ...
    Ta.ffgp = 100; eps.ffgp=1e-8; ucb.val.ffgp = vector()
    ...
    for(l.ffgp in 1:Ta.ffgp){
      ...
      sr.ffgp = 0.003/sqrt(l.ffgp)#/sqrt(mean(grad.old))
      ...
      # d.ucb.dis.ffgp = jacobian(func=ucb.ffgp, x=dis.pre.0) 
      d.k.dis.ffgp = jacobian(function(dis) k.x.ffgp(dis,hyper.ffgp.new[2]), dis.pre.0)
      d.ucb.dis.ffgp = d.k.dis.ffgp%*%re.ucb.val.ffgp$d.ucb.k
...
save(re.ffgp, file="s3-re-ffgp1.RData")
