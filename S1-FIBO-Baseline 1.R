...
for(j.figp in 1:J.figp){
  ...
  # x.da.figp <- function(s0) apply(x.for.figp[[j.figp]],1,function(y.row) x.sp(y.row,s0))
  ...
  for(i.figp in 1:m){
    ...
    Ta.figp = 100; eps.figp=1e-6; ucb.val.figp = vector()
    ...
    for(l.figp in 1:Ta.figp){
      ...
      sr.figp = 0.01#/sqrt(mean(grad.old))
...
save(re.figp, file="s1-re-figp.RData")  
